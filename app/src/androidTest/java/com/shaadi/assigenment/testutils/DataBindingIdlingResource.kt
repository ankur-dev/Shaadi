package com.shaadi.assigenment.testutils
...
class DataBindingIdlingResource(
    private val activityTestRule: ActivityScenarioRule<*>
) : IdlingResource {
    ...
    override fun isIdleNow(): Boolean {
        ...
        if (idle) {
            ...
            wasNotIdle = true
            // check next frame
            activityTestRule.scenario.onActivity { it.findViewById<View>(android.R.id.content).postDelayed({
                isIdleNow
            }, 16) }
        }
        return idle
    ...
    private fun getActivtyBinding(): List<ViewDataBinding> {
        ...
        activityTestRule.scenario.onActivity {
            contentView = it.window.decorView.findViewById(android.R.id.content) as ViewGroup
        }



        val childs = contentView?.childCount?:0
